...
#include "Utility.h"
bool AssignLighting::StartUp()
{
	LoadShaders("./Shaders/Lighting_Vertex.glsl", nullptr, "./Shaders/Lighting_Fragmant.glsl", &m_lightingID);
...
void AssignLighting::reloadShader()
{
	glDeleteProgram(m_lightingID);
	LoadShaders("./Shaders/Lighting_Vertex.glsl", nullptr, "./Shaders/Lighting_Fragmant.glsl", &m_lightingID);
}

bool AssignLighting::Update()
{
...
void AssignLighting::ShutDown()
{
	CleanUpOpenGLBuffers();
}
void AssignLighting::Draw(FlyCamera a_camera)
{
	//glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glUseProgram(m_lightingID);
	//Gizmos::draw(m_Camera.getProjectionView());

	int proj_veiw_uniform = glGetUniformLocation(m_lightingID, "projection_view");
	glUniformMatrix4fv(proj_veiw_uniform, 1, GL_FALSE, (float*)&a_camera.getProjectionView());

	int ambiant_Uniform = glGetUniformLocation(m_lightingID, "ambient_light");
	int LightDir_Uniform = glGetUniformLocation(m_lightingID, "light_dir");
	int LightColour_Uniform = glGetUniformLocation(m_lightingID, "light_colour");
	int MatirialColour_Uniform = glGetUniformLocation(m_lightingID, "material_colour");
	int cameraPost_Uniform = glGetUniformLocation(m_lightingID, "eye_pos");
	int specPower_Uniform = glGetUniformLocation(m_lightingID, "spec_power");
	...
	vec3 camera_pos = a_camera.m_WorldTransform[3].xyz;
...
void AssignLighting::CreateOpenGLBuffers(std::vector<tinyobj::shape_t>& shapes)
{
...
void AssignLighting::CleanUpOpenGLBuffers()
{
	glDeleteProgram(m_lightingID);
